...
UnflattenVolumeProcess::UnflattenVolumeProcess( AVOProject* project, QObject* parent) :
    ProjectProcess( QString("Unflatten Volume"), project, parent){

}

ProjectProcess::ResultCode UnflattenVolumeProcess::init( const QMap<QString, QString>& parameters ){
    ...
    auto out_bounds=Grid3DBounds( bounds.i1(), bounds.i2(), bounds.j1(), bounds.j2(), out_nt, 0.001*m_start, bounds.dt());
...
ProjectProcess::ResultCode UnflattenVolumeProcess::run(){
    ...
    for( int i=bounds.i1(); i<=bounds.i2(); i++){
        ...
        for( int j=bounds.j1(); j<=bounds.j2(); j++){
            ...
            for( int k=0; k<bounds.nt(); k++){
                ...
                (*m_volume)(i,j,k)=m_inputVolume->value( i, j, tflat);  // this doesinterpolation
            }
        }
